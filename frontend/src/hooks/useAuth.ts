...
export function useAuth() {
  ...
  const fetchOrCreateStats = async (userId: string) => {
    ...
    if (!hasUserStatsTable) {
      const localKey = `flick_stats_${userId}`;
      ...
      if (stored) {
        ...
        localStorage.setItem(localKey, JSON.stringify(defaultStats));
    ...
    try {
      ...
      if (error) {
        if (error.code === 'PGRST205' || error.message?.includes('Could not find the table') || error.message?.includes('does not exist')) {
          hasUserStatsTable = false;
          fetchOrCreateStats(userId);
          return;
        }
        throw error;
      }
      ...
      if (data) {
        ...
        if (insertError) {
          if (insertError.code === 'PGRST205' || insertError.message?.includes('Could not find the table') || insertError.message?.includes('does not exist')) {
            hasUserStatsTable = false;
            fetchOrCreateStats(userId);
            return;
          }
      ...
      if (err.code === 'PGRST205' || err.message?.includes('Could not find the table') || err.message?.includes('does not exist')) {
        hasUserStatsTable = false;
        fetchOrCreateStats(userId);
        return;
      }
  ...
  const updateStudyStats = async (deckTitle: string, cardsReviewed: number) => {
    if (!user) return;
    ...
    if (isDemoMode || !hasUserStatsTable) {
      const fullUpdatedStats: UserStats = {
        ...stats!,
        ...updated,
      };
      const localKey = isDemoMode ? 'flick_demo_stats' : `flick_stats_${user!.id}`;
      localStorage.setItem(localKey, JSON.stringify(fullUpdatedStats));
      setStats(fullUpdatedStats);
      
      // Update recent activity in local storage
      const recentKey = isDemoMode ? 'flick_demo_recent_activity' : `flick_recent_activity_${user!.id}`;
      ...
      try {
        ...
        if (error) {
          if (error.code === 'PGRST205' || error.message?.includes('Could not find the table') || error.message?.includes('does not exist')) {
            hasUserStatsTable = false;
            // Retry update locally
            updateStudyStats(deckTitle, cardsReviewed);
            return;
          }
          throw error;
        }
        ...
        // Also update recent activity locally for rendering widgets
        const recentKey = `flick_recent_activity_${user.id}`;
        const currentRecent = JSON.parse(localStorage.getItem(recentKey) || '[]');
        const newActivity = {
          id: Math.random().toString(36).substr(2, 9),
          deck_title: deckTitle,
  ...
  const clearAllUserData = async () => {
    ...
    try {
      ...
      // Delete user stats if table exists, otherwise clean local storage
      if (hasUserStatsTable) {
        const { error: statsError } = await supabase.from('user_stats').delete().eq('user_id', user!.id);
        if (statsError && statsError.code !== 'PGRST205' && !statsError.message?.includes('Could not find the table')) {
          throw statsError;
        }
      }
      localStorage.removeItem(`flick_stats_${user!.id}`);
      localStorage.removeItem(`flick_recent_activity_${user!.id}`);
