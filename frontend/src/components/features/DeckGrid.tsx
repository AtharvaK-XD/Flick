...
export function DeckGrid({ decks, allCards }: DeckGridProps) {
  ...
  const getDueBadgeClass = (count: number) => {
    if (count === 0) return 'bg-surface-2 text-[var(--text-muted)] border-[var(--border)]';
  ...
  if (decks.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center border border-[var(--border)] bg-surface rounded-xl p-8">
